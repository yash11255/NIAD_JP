...
const ViewApplications = () => {
  ...
  const jobTitles = [...new Set(viewApplicationsPageData.map((app) => app.jobTitle))];
  ...
  return (
    <div className="p-4 md:p-6 font-sans">
      <h2 className="text-center text-2xl md:text-3xl font-bold mb-6">Job Applications</h2>
        ...
        <div className="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-5">
          ...
          <ul className="divide-y divide-gray-300">
            {jobTitles.map((job, index) => (
              <li key={index} className="p-4 bg-blue-100 hover:bg-blue-200 cursor-pointer transition rounded-lg mb-2"
      ...
      {selectedJob && !selectedCandidate && (
        <div className="mt-6">
          <h3 className="text-center text-xl font-semibold mb-4">{selectedJob} - Candidates</h3>
          <button className="block mx-auto bg-gray-500 text-white px-4 py-2 rounded-md mb-4 hover:bg-gray-600 transition"
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-blue-500 text-white">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">10th Board</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">10th Year</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">10th %</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">12th Board</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">12th Year</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">12th %</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Qualification</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Status</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Onboard Status</th>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Interview Status</th>
                    ...
                    <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${statuses[app.userId] === "Accepted" ? "bg-green-100 text-green-800" : 
                            statuses[app.userId] === "Rejected" ? "bg-red-100 text-red-800" : 
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <select 
                          ...
                          onChange={(e) => handleOnboardStatusChange(app.userId, e.target.value)}
                        >
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <select 
                          ...
                          onChange={(e) => handleInterviewStatusChange(app.userId, e.target.value)}
                        >
                          <option value="Pending">Pending</option>
      ...
      {selectedCandidate && (
        <div className="mt-6">
          <div className="flex justify-between items-center mb-4">
            ...
            >
              <ChevronLeft className="inline mr-1" size={18} /> Back to Candidates
            </button>
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="p-6 border-b">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedCandidate.aadhaarNumber || "N/A"}</p>
                </div>
                <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                    <div>
                      ...
                      <p className="font-medium">{selectedCandidate.education?.tenth?.board || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Year</p>
                      <p className="font-medium">{selectedCandidate.education?.tenth?.year || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Percentage</p>
                      <p className="font-medium">{selectedCandidate.education?.tenth?.percentage || "N/A"}%</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                    <div>
                      <p className="text-sm text-gray-500">Board</p>
                      <p className="font-medium">{selectedCandidate.education?.twelfth?.board || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Year</p>
                      <p className="font-medium">{selectedCandidate.education?.twelfth?.year || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Percentage</p>
                      <p className="font-medium">{selectedCandidate.education?.twelfth?.percentage || "N/A"}%</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    <div>
                      ...
                      <p className="font-medium">{selectedCandidate.education?.graduation?.degree || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">University</p>
                      <p className="font-medium">{selectedCandidate.education?.graduation?.university || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Year of Graduation</p>
                      <p className="font-medium">{selectedCandidate.education?.graduation?.yearOfGraduation || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">CGPA</p>
                      <p className="font-medium">{selectedCandidate.education?.graduation?.cgpa || "N/A"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="mt-2">{selectedCandidate.education?.certifications?.join(", ") || "None"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h5 className="font-medium text-blue-600 mb-2">Permanent Address</h5>
                  <p>{selectedCandidate.permanentAddress?.street}</p>
                  <p>{selectedCandidate.permanentAddress?.city}, {selectedCandidate.permanentAddress?.state}</p>
                  <p>{selectedCandidate.permanentAddress?.pincode}</p>
                </div>
                <div>
                  <h5 className="font-medium text-blue-600 mb-2">Current Address</h5>
                  {selectedCandidate.currentAddress?.city === selectedCandidate.permanentAddress?.city ? (
                    <p>Same as Permanent Address</p>
                  ) : (
                    <>
                      <p>{selectedCandidate.currentAddress?.street}</p>
                      <p>{selectedCandidate.currentAddress?.city}, {selectedCandidate.currentAddress?.state}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => setStatuses(prev => ({ ...prev, [selectedCandidate.userId]: e.target.value }))}
                  >
                    <option value="Pending">Pending</option>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Onboard Status</label>
                  <select 
                    className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                    value={onboardStatuses[selectedCandidate.userId] || "Pending"}
                    onChange={(e) => handleOnboardStatusChange(selectedCandidate.userId, e.target.value)}
                  >
                    <option value="Pending">Pending</option>
                    <option value="In Progress">In Progress</option>
                    <option value="Onboarded">Onboarded</option>
                ...
                <div>
                  ...
                  <select 
                    className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                    value={interviewStatuses[selectedCandidate.userId] || "Pending"}
                    onChange={(e) => handleInterviewStatusChange(selectedCandidate.userId, e.target.value)}
                  >
                    <option value="Pending">Pending</option>
                    <option value="Scheduled">Scheduled</option>
                    <option value="Completed">Completed</option>
