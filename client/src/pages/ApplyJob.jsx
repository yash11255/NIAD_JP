            ...
            <>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b pb-6">
                <div className="flex items-center gap-6">
                  ...
                  />
                  <div>
                    <h1 className="text-3xl font-semibold text-gray-800">{jobsData.title}</h1>
                    <p className="text-gray-500">{jobsData.companyId.name}</p>
                    <div className="flex flex-wrap gap-2 mt-2 text-sm">
                      <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-md">📍 {jobsData.location}</span>
                      <span className="bg-red-100 text-red-700 px-3 py-1 rounded-md">⚡ {jobsData.level}</span>
                      <span className="bg-green-100 text-green-700 px-3 py-1 rounded-md">💰 CTC: {jobsData.salary}</span>
                    </div>
                  </div>
                </div>
                <div className="text-center md:text-right">
                  ...
                  <p className="text-gray-500 text-sm mt-2">Posted {moment(jobsData.date).fromNow()}</p>
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="text-gray-700 mt-3 leading-relaxed" dangerouslySetInnerHTML={{ __html: jobsData.description }}></div>
              </div>

              {/* Responsibilities & Requirements */}
              <div className="mt-6">
                <h2 className="text-2xl font-semibold text-gray-800">Responsibilities & Requirements</h2>
              ...
              <div className="mt-6 border-t pt-4 text-gray-600 flex flex-wrap justify-between">
                <p>📅 Application Deadline: <span className="font-medium">{moment(jobsData.deadline).format("MMMM Do, YYYY")}</span></p>
                <p>🏢 Work Type: <span className="font-medium">{jobsData.workType}</span></p>
              </div>
            </>
          ) : (
